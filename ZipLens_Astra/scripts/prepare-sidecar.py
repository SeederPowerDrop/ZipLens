...
# Source and full license accompany every redistributed executable. A missing
# or substituted source archive must stop packaging rather than silently omit it.
for record in [provenance['source'], provenance['license_text']]:
    path = root / record['file']
    if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest() != record['sha256']:
        raise SystemExit(f'Missing or changed distribution resource: {record["file"]}')
legal = root / 'legal/7zip'
...
with tarfile.open(archive) as source:
    for name in ['7zz', 'License.txt', 'History.txt', 'readme.txt']:
        ...
        if name in ['License.txt', 'readme.txt']:
            target = legal / name
